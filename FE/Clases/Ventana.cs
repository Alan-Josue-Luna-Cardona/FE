...
namespace NaveEspacial
{
    internal class Ventana
    {
        ...
        private void Base()//sera nuestro tablero
        {
            ...
            Console.Clear();
            _enemigo1 = new Enemigo(new Point(50, 10), ConsoleColor.Cyan, this, TipoEnemigo.Menu, null);//Enemigos tipo menu son decoracion
            _enemigo2 = new Enemigo(new Point(100, 30), ConsoleColor.Red, this, TipoEnemigo.Menu, null);
            _balas = new List<Bala>();//Llamamos las balas de adorno
            CrearBalas();
        }
        public void DibujarMarco()//Bordes del tablero
        {
        ...
        public void Peligro()// pantalla de siguiente nivel
        {
            Console.Clear();
            DibujarMarco();
            for (int i = 0; i < 6; i++)
            {
                ...
                Console.SetCursorPosition(LimiteInferior.X / 2 - 5, LimiteInferior.Y / 2);
                Console.Write("           ");
                Thread.Sleep(200);//Reduce el tiempo de ejecución o los frames
            }
        }
        public void Menu()
        {
            _enemigo1.Mover();//movemos los enemigos de adorno
            _enemigo2.Mover();
            MoverBalas();
            Console.ForegroundColor = ConsoleColor.White;//Menu de de seleccion
            Console.SetCursorPosition(LimiteInferior.X / 2 - 5, LimiteInferior.Y / 2 - 1);
            Console.Write("[Enter] JUGAR");
            Console.SetCursorPosition(LimiteInferior.X / 2 - 5, LimiteInferior.Y / 2);
            Console.Write("[Esc] SALIR");
        }
        public void Teclado(ref bool ejecucion, ref bool jugar)//Lee lo que ingresemos en el menu de seleccion
        {
        ...
        public void CrearBalas()//Creamos las balas de decoración. Una linea por cada color
        {
